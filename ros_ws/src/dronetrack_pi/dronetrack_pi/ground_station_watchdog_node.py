...
class GroundStationWatchdogNode(Node):
    ...
    def _tick(self) -> None:
        ...
        # Edge detection: react once on transitions.
        if self._link_initialized:
            if self._link_ok and not link_ok:
                self._on_link_lost(reason)
            elif not self._link_ok and link_ok:
                self.get_logger().info(f"Ground station link RECOVERED ({reason}).")
