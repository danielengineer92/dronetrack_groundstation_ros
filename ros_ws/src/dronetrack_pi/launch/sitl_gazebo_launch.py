...
def _create_bridge_and_spawn(context, *args, **kwargs):
    ...
    # ---- write bridge config (services only; the camera uses image_bridge) ----
    ...
    # ---- ros_gz_bridge (set_pose service only) ----
    actions.append(Node(
    ...
    # ---- camera via the dedicated image bridge ----
    # ros_gz_image's image_bridge is purpose-built for image streams and is more
    # robust than parameter_bridge for large messages (the generic bridge has a
    # history of silently stalling on the camera topic; see sim_watchdog.sh for
    # the recovery of the gz-server-side variant of that hang).
    actions.append(Node(
        package="ros_gz_image",
        executable="image_bridge",
        name="gz_image_bridge",
        arguments=[cam_gz],
        remappings=[(cam_gz, cam_ros)],
        output="screen",
    ))
